...
:
    ...
    private def calcPlayerHit(b:Vector3, p:Vector3) =
        ...
        if ac.x < thresh && ac.x >= ac.y && ac.x >= ac.z then
            new Vector3(if c.x > 0 then b.x + 1 - shift else b.x + shift, p.y, p.z)
        else if ac.y < thresh && ac.y >= ac.z && ac.y >= ac.x then
            new Vector3(p.x, if c.y > 0 then b.y + 1 - shift else b.y + shift, p.z)
        else if ac.z < thresh && ac.z >= ac.x && ac.z >= ac.y then
            new Vector3(p.x, p.y, if c.z > 0 then b.z + 1 - shift else b.z + shift)
        else p
    ...
    private def raytraceModel(from:Vector3, dir:Vector3, model:CCModel) =
        ...
        {
            case Some(dist) =>
                if currentHit == None || dist < currentHit.get._1 then
