...
class PanNode extends TNode
{
    ...
    override def frameUpdate_Impl(mouse:Point, rframe:Float): Unit =
        ...
        if mouseDownRight || mouseDownBelow then
            val sf2 = size.vectorize/cFrame.size.vectorize
            val modVec = if mouseDownRight then Vec2.up else Vec2.left
            ...
            //x disp
            val l = 0+clampSlack
            val lc = if cFrame.size.width > size.width then cFrame.origin.x max l else cFrame.origin.x min l
            val ld = l-lc
            val r = size.width-clampSlack
            val rc = if cFrame.size.width > size.width then cFrame.maxX min r else cFrame.maxX max r
            val rd = r-rc

            //y disp
            val t = 0+clampSlack
            val tc = if cFrame.size.height > size.height then cFrame.origin.y max t else cFrame.origin.y min t
            val td = t-tc
            val b = size.height-clampSlack
            val bc = if cFrame.size.height > size.height then cFrame.maxY min b else cFrame.maxY max b
            val bd = b-bc
    ...
    def panChildren(d:Vec2): Unit =
        val d2 = d
        if d2 != Vec2.zeroVec then
            for c <- children do c.position = Point(c.position.vectorize+d2)
            panDelegate()
    ...
    def getScrollBarRight:Rect =
        if cFrame.size.height == 0 then return Rect.zeroRect
    ...
    def getScrollBarBelow:Rect =
        if cFrame.size.width == 0 then return Rect.zeroRect
    ...
    override def mouseClicked_Impl(p:Point, button:Int, consumed:Boolean):Boolean =
        ...
        if !consumed then
            if scrollBarVertical && doRayTest(1) then mouseDownRight = true
            else if scrollBarHorizontal && doRayTest(2) then mouseDownBelow = true
            else if doRayTest(3) then mouseDown = true
            else return false
