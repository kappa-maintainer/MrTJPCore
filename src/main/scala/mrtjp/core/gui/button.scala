...
:
    abstract override def drawButtonBackground(mouseover:Boolean): Unit =
        ...
        GlStateManager.color(1, 1, 1, 1)
        val state = if mouseover then 2 else 1
...
:
    ...
    abstract override def drawButton(mouseover:Boolean): Unit =
        ...
        GuiDraw.drawStringC(text, position.x+size.width/2, position.y+(size.height-8)/2, if mouseover then 0xFFFFFFA0 else 0xFFE0E0E0)
        GlStateManager.color(1, 1, 1, 1)
...
:
    ...
    override def drawButtonBackground(mouseover:Boolean): Unit =
        ...
        GlStateManager.color(1, 1, 1, 1)
        drawTexturedModalRect(position.x, position.y, if mouseover then 11 else 1, 1, 8, 8)
