...
:
    ...
    def pageUp(): Unit =
        currentPage += 1
        if currentPage > pagesNeeded then currentPage = pagesNeeded
    ...
    private def getSeachedCount =
        var count = 0
        for stack <- displayList do if filterAllows(stack) then count += 1
        count
    ...
    override def drawBack_Impl(mouse:Point, frame:Float): Unit =
        ...
        if pagesNeeded < 0 then pagesNeeded = 0
        if currentPage > pagesNeeded then currentPage = pagesNeeded

        if !downloadFinished then drawLoadingScreen()
        else drawAllItems(mouse.x, mouse.y)

    override def drawFront_Impl(mouse:Point, rframe:Float): Unit =
        if hover != null then GuiDraw.drawMultiLineTip(
    ...
    private def drawLoadingScreen(): Unit =
        ...
        val time = System.currentTimeMillis/(if waitingForList then 40 else 8)
        val percent = (time%barSizeX).asInstanceOf[Int]

        if !waitingForList && percent > barSizeX-8 then downloadFinished = true
    ...
    private def drawAllItems(mx:Int, my:Int): Unit =
        ...
        b.breakable
          :
            for keystack <- displayList do c.breakable
              :
                if !filterAllows(keystack) then c.break()
                itemNumber += 1
                if itemNumber <= rows*columns*currentPage then c.break()
                if itemNumber > (rows*columns)*(currentPage+1) then b.break()

                val localX = xOffset+renderPointerX*squareSize
                val localY = yOffset+renderPointerY*squareSize
                if mx > localX && mx < localX+squareSize && my > localY && my < localY+squareSize then hover = keystack
                if xLast > localX && xLast < localX+squareSize && yLast > localY && yLast < localY+squareSize then selection = keystack

                if selection != null && (selection == keystack) then
                ...
                if renderPointerX > columns then
                    renderPointerX = 1
                    renderPointerY += 1
                if renderPointerY > rows then b.break()
        glItemPost()
