...
class WorldGenCaveReformer extends TWorldGenerator:
    ...
    override def generate(w: World, rand: Random, pos: BlockPos): Boolean =
        if !w.isAirBlock(pos) then return false
        var dy = 0
        while dy < searchRadius && !canSetBlock(w, pos.add(0, dy, 0), material) do dy += 1
        if !canSetBlock(w, pos.add(0, dy, 0), material) then return false
    ...
    private def iterate(w: World, open: Seq[Node], closed: Set[Node] = Set.empty, generated: Boolean = false): Boolean = open match
        ...
        case Seq(next, rest@_*) =>
            if closed.size > clusterSize then return generated
            val g2 = setBlock(w, next.bc, cluster, material)
            val upNext = Vector.newBuilder[Node]
            if next.depth > 0 then
                for s <- 0 until 6 do
                    val to = next --> EnumFacing.values()(s)
                    if !open.contains(to) && !closed.contains(to) then
                        to.depth = if WorldLib.isBlockTouchingAir(w, to.bc) then depth else next.depth - 1
                        upNext += to
