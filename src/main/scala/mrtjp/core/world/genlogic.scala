...
:
    ...
    def preFiltCheck(w:World, chunkX:Int, chunkZ:Int, rand:Random, isRetro:Boolean):Boolean =
        if isRetro && !allowRetroGen then return false
        if dimensionBlacklist == dimensions.contains(w.provider.getDimension) then return false
        if typeBlacklist == types.contains(w.getWorldInfo.getTerrainType) then return false
        if resistance > 1 && rand.nextInt(resistance) != 0 then return false
        true

    def postFiltCheck(w:World, pos:BlockPos, rand:Random):Boolean =
        val types = BiomeDictionary.getTypes(w.getBiome(pos))
        if biomeBlacklist == biomes.asJava.contains(types) then return false
        true
    ...
    override def generate(w:World, chunkX:Int, chunkZ:Int, rand:Random, isRetro:Boolean):Boolean =
        if !preFiltCheck(w, chunkX, chunkZ, rand, isRetro) then return false
...
:
    ...
    protected def canSetBlock(w:World, pos:BlockPos, material:Set[(Block, Int)]):Boolean =
        if material.isEmpty then return true
    ...
    protected def setBlock(w:World, pos:BlockPos, cluster:Set[((Block, Int), Int)], material:Set[(Block, Int)]):Boolean =
        if canSetBlock(w, pos, material) then
...
:
    ...
    override def generate_impl(w:World, chunkX:Int, chunkZ:Int, rand:Random) =
        ...
        for i <- 0 until attempts do
            ...
            if postFiltCheck(w, pos, rand) then
                pos = WorldLib.findSurfaceHeight(w, pos)
                if pos.getY > 0 then generated |= gen.generate(w, rand, pos)
        generated
