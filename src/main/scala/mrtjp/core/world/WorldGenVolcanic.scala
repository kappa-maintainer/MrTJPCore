...
:
    ...
    override def generate(w:World, rand:Random, pos:BlockPos):Boolean =
        if !canSetBlock(w, pos, materialStart) then return false
        stack.clear()
        test.clear()
        ...
        breakable(while size > 0 do
        {
            ...
            if w.isBlockLoaded(new BlockPos(i, 64, k)) && test.contains((i, k)) then
            {
                var pow = test((i, k))
                var hm = w.getHeight(i, k)+1
                while hm > 0 && isUnimportant(w, i, hm-1, k) do hm -= 1

                if hm <= j then if isUnimportant(w, i, hm, k) then
                {
                    purgeArea(w, i, hm, k)
                    setBlock(w, new BlockPos(i, hm, k), ashCluster, material)
                    if j > hm then pow = math.max(pow, spread)
        ...
        while n >= swh && liq._1 == w.getBlockState(p).getBlock do
    ...
    private def purgeArea(w:World, x:Int, y:Int, z:Int): Unit =
        if w.isAirBlock(new BlockPos(x, y, z)) then return
        for i <- -1 to 1 do for j <- -1 to 1 do
            val p = new BlockPos(x+i, y, z+j)
            val b = w.getBlockState(p)
            if (b.getBlock == Blocks.SNOW) || WorldLib.isAssociatedTreeBlock(w, p, b) then
                w.setBlockToAir(p)
        purgeArea(w, x, y+1, z)

    private def enqueueBlocks(x:Int, y:Int, z:Int, p:Int, rand:Random): Unit =
        val seed = rand.nextInt(16)
        enq(x-1, y, z, if (seed&1) != 0 then p-1 else p)
        enq(x+1, y, z, if (seed&2) != 0 then p-1 else p)
        enq(x, y, z-1, if (seed&4) != 0 then p-1 else p)
        enq(x, y, z+1, if (seed&8) != 0 then p-1 else p)
    ...
    private def isUnimportant(w:World, x:Int, y:Int, z:Int):Boolean =
        ...
        val b = s.getBlock
        if WorldLib.isBlockSoft(w, p, s) || WorldLib.isAssociatedTreeBlock(w, p, s) then return true
        if b == Blocks.FLOWING_WATER || b == Blocks.WATER ||b == Blocks.SNOW || b == Blocks.ICE then return true
        false
