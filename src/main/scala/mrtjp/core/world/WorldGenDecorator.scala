...
:
    ...
    override def generate(w:World, rand:Random, pos:BlockPos) =
        ...
        val zStart = pos.getZ
        var generated = false
        for i <- 0 until cluster.size do
            val x = xStart+rand.nextInt(dx)-rand.nextInt(dx)
            var y = yStart+1+(if dy > 1 then rand.nextInt(dy)-rand.nextInt(dy) else 0)
            val z = zStart+rand.nextInt(dz)-rand.nextInt(dz)

            if checkLocation(w, x, y, z) then
                ...
                breakable(for s <- 0 to h do
                {
                    ...
                    y += 1
                    if !canSetBlock(w, new BlockPos(x, y, z), material) then break()
                })
        generated

    def checkLocation(w:World, x:Int, y:Int, z:Int):Boolean =
        val pos = new BlockPos(x,y,z)
        if seeSky && !w.canSeeSky(pos) then return false
        if !canSetBlock(w, pos, material) then return false
        if !canSetBlock(w, pos.down(), soil) then return false
        true
