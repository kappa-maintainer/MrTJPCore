...
object MathLib
{
    ...
    def shift(dir:Int) = if (dir&1) == 1 then 1 else -1

    def splitLine(xs:Seq[Int], shift:Int) =
        if xs.isEmpty then Seq()
        else
            var start = 0
            val ret = for
    ...
    def bezier(s:Vector3, c1:Vector3, c2:Vector3, e:Vector3, t:Float):Vector3 =
        if (t < 0.0F) || (t > 1.0F) then return s
        ...
        for i <- 0 until 4 do retValue.add(terms(i))
        retValue
    ...
    def weightedRandom[T](xs:Iterable[(T, Int)], rand:Random = random):T =
        if xs.size == 1 then return xs.head._1
